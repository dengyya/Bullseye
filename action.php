  ...
  <div class="container-fluid">
    <div class="row justify-content-center">
      ...
      <div class="col-11 col-sm-9 col-md-7 col-lg-6 col-xl-5 text-center p-4 mt-3 mb-2 ">
        ...
        <div class="card pt-5 card-timeline px-2 border-none mb-4">
          ...
          <div class="card-deck">
            <div class="  col-lg-6 offset-md-3 mb-4 ">
              <img class="card-img-top" src="img/action.png" style="width:100px" alt="Card image cap">
              <div class="card-body">
                <p class="card-text">3. ACTION MADE - Action to your report is completed.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        $sql_get = mysqli_query($conn, "SELECT * FROM complaints where complainant_id = {$_SESSION['login_id']} and status = 3");
              ...
              <div class="container mt-4">
                <table class="table   table-hover">
                  ...
                  <tbody>
                    <?php
                    $i = 1;
                    $qry = $conn->query("SELECT * FROM complaints where complainant_id = {$_SESSION['login_id']} and status = 3 order by unix_timestamp(date_created) desc ");
                    if ($qry->num_rows > 0) :
        ...
        $sql_get = mysqli_query($conn, "SELECT * FROM crime where complainant_id = {$_SESSION['login_id']} and status = 3");
                ...
                <div class="container">
                  <table class="table table-hover">
                    ...
                    <tbody>
                      <?php
                      $i = 1;
                      $qry = $conn->query("SELECT * FROM crime where complainant_id = {$_SESSION['login_id']} and status = 3 order by unix_timestamp(date_created) desc ");
                      if ($qry->num_rows > 0) :
                        while ($row = $qry->fetch_array()) :
                      ?>
                          <tr>
        ...
        $sql_get = mysqli_query($conn, "SELECT * FROM missing where complainant_id = {$_SESSION['login_id']} and status = 3");
                ...
                <div class="container">
                  <table class="table   table-hover">
                    ...
                    <tbody>
                      <?php
                      $i = 1;
                      $qry = $conn->query("SELECT * FROM missing where complainant_id = {$_SESSION['login_id']} and status = 3 order by unix_timestamp(date_created) desc ");
                      if ($qry->num_rows > 0) :
                        while ($row = $qry->fetch_array()) :
                      ?>
                          <tr>
